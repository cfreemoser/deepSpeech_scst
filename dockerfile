...
RUN echo 'whatever force rebuild agian agian agian agian agian agian' && git clone https://github.com/SpoC-DHKA/DeepSpeech.git
...
###
# # Build TF pip3 package
# RUN bazel build --config=opt --config=cuda --copt="-D_GLIBCXX_USE_CXX11_ABI=0" --copt=-mtune=generic --copt=-march=x86-64 --copt=-msse --copt=-msse2 --copt=-msse3 --copt=-msse4.1 --copt=-msse4.2 --copt=-mavx //tensorflow/tools/pip3_package:build_pip3_package --verbose_failures --action_env=LD_LIBRARY_PATH=${LD_LIBRARY_PATH}
#
# # Build wheel
# RUN bazel-bin/tensorflow/tools/pip3_package/build_pip3_package /tmp/tensorflow_pkg
#
...
WORKDIR /checkpoints
RUN wget https://github.com/mozilla/DeepSpeech/releases/download/v0.4.1/deepspeech-0.4.1-checkpoint.tar.gz
RUN tar -xvzf deepspeech-0.4.1-checkpoint.tar.gz
